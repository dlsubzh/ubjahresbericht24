...
r = new Rive({
  ...
  const eventHandlers = {
    ...
    "QuotePointerEnter": (data) => {
      handleQuotePointerEnter(data)
    },
...
const Quotes = {
  "Quote1": "Veränderungen sind richtig – wichtig sind die Menschen. Bereichernd war und ist die unkomplizierte Zusammenarbeit und Wertschätzung in unserem Team und insgesamt in der UB. Persönliches Highlight war, dass ich an der Entwicklung des Leitfadens schriftliche Kund*innen-Kommunikation mitwirken durfte.",
  ...
  "Quote10": "2024 ist die UB aus meiner Sicht nochmals zusammengerückt. Die Zusammenarbeit ist durch die dezentrale Struktur nach wie vor komplex, aber die Rollen und Wege werden immer klarer.",
...
function handleQuoteChange(event) {
  // Get current quote key
  const currentKey = quoteKeys[currentQuoteIndex];

  // Get the current quote text
  const currentQuoteText = Quotes[currentKey];

  // Set the text run value with the current quote
  r.setTextRunValue("Quotes_TextRun", currentQuoteText);
...
function handleQuotePointerEnter(event) {

  let name = event.name;

  console.log(`Handling ${name}:`);
  
  r.pause("QuotesTracking");
